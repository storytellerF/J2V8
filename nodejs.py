...
def git_checkout(args):
    ...
    if os.path.exists("node"):
        ...
        utils.execute("git checkout --progress tags/v" + settings.NODE_VERSION + " -b v" + settings.NODE_VERSION,
                      "node")
    else:
        print(
            "ERROR: Node.js source-code was not yet cloned into the './node' directory, run 'python nodejs.py "
            "git-clone' first.")
...
def package(platforms=None):
    ...
    full = platforms is None or len(platforms) == 0
    ...
    with tarfile.open(fileobj=WriteProgressFileObject(package_filename, dependencies["size"], "w"),
                      mode="w:bz2") as zipf:
